...
(function () {
  ...
  */
  function cssToRgba (color) {
    // make a dummy element and set its color
    let tempElement = document.createElement('div');
    tempElement.style.color = color;
    
    // ensure it doesn't mess up the page
    tempElement.style.display = 'none';
    
    // add it to the body
    document.body.appendChild(tempElement);
    
    // ask the window for the rgba
    let rgba = window.getComputedStyle(tempElement).getPropertyValue("color");

    // axe the dummy element
    tempElement.remove();

    // "rgb(30, 50, 220)" -> [30, 50, 220]
    let [r, g, b, a] = rgba
      .match(/[0-9.]+/g)
      .map(Number);
  ...
  window.compressBitmap = function (data) {
    ...
    Object.keys(data.p)
      ...
      .map(c => {
        ...
        return (((a * 255) << 24) & 4278190080 | (r << 16) & 16711680 | (g << 8) & 65280 | b & 255) + c;
      })
